    ...
    try {
      ...
      if (data.success) {
        setStats({
          ...
          resellerId: data.affiliate?.resellerId || '',
    ...
    try {
      const response = await fetch('/api/affiliate/referrals', {
        ...
        body: JSON.stringify({
          ...
          estimatedValue: parseFloat(submitForm.estimatedValue) || 0,
        }),
      });
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card className="glass-card border-0">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <div className="space-y-1">
                ...
                <CardDescription>Daily commission trends</CardDescription>
              </div>
        ...
        >
          <Card className="glass-card border-0">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <div className="space-y-1">
                <CardTitle className="text-lg font-bold">Clicks & Engagement</CardTitle>
                <CardDescription>Referral traffic performance</CardDescription>
              </div>
            ...
            <>
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input readOnly value={stats?.resellerId || ''} className="font-mono text-sm font-semibold bg-beam-pink-50 text-beam-pink-700 border-beam-pink-200" />
                  <Button
                    variant="outline"
                    size="icon"
                    onClick={() => copyToClipboard(stats?.resellerId || '', 'code')}
                  >
                    {copied === 'code' ? <Check className="h-4 w-4 text-emerald-600" /> : <Copy className="h-4 w-4" />}
                  </Button>
                </div>
                <p className="text-xs text-muted-foreground">Your unique Beam reseller identifier</p>
              </div>
              <div className="space-y-2">
