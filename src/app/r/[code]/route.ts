...
export async function GET(
  ...
  try {
    ...
    const resellerOrReferralCode = decodeURIComponent(code);
    const searchParams = request.nextUrl.searchParams;
    const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://app.beamaffiliate.com';
    ...
    // Find affiliate by strict reseller ID first, then legacy referral code.
    const affiliate = await prisma.affiliate.findFirst({
      where: {
        OR: [
          { resellerId: resellerOrReferralCode },
          { referralCode: resellerOrReferralCode },
        ],
      },
      include: { user: true }
    });
    ...
    redirectUrl.searchParams.set('reseller', affiliate.resellerId);
    redirectUrl.searchParams.set('ref', affiliate.referralCode);
    ...
    response.cookies.set('affiliate_attribution', JSON.stringify({
      ...
      reseller_id: affiliate.resellerId,
      attribution_key: attributionKey,
