  ...
  try {
    ...
    try {
      ...
      if (adminIds.length > 0) {
        await notificationService.notifyReferralSubmitted({
          affiliateName: user.name || 'A Partner',
          leadName: leadName,
          company: company,
        }, adminIds);
      }
      ...
      await emailService.sendReferralNotification(process.env.ADMIN_EMAIL || 'admin@beamwallet.com', {
        affiliateName: user.name || 'A Partner',
        leadName: leadName,
        leadEmail: leadEmail,
        company: company,
