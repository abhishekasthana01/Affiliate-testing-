...
export async function GET(request: NextRequest) {
  try {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayClicks = referrals
        .filter(r => r.createdAt.toISOString().split('T')[0] === dateStr)
        .reduce((sum, r) => {
          const metadata = r.metadata as any;
          return sum + (metadata?.clicks || 1); // Default to 1 if not tracked
        }, 0);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { name, company, email, phone, beamNumber, country, paymentMethod, paymentEmail } = body;
    ...
    if (beamNumber !== undefined) {
      userUpdateData.beamNumber = beamNumber?.trim() || null;
    }
