...
export async function GET(request: NextRequest) {
  try {
    ...
    // Confirmed revenue should only include approved conversions.
    ...
    const partnerGroupMap = new Map(
      partnerGroups.map(pg => [pg.id, normalizeCommissionRate(pg.commissionRate)])
    );
    ...
    referrals.forEach((ref) => {
      ...
      const commissionRate = affiliate.commissionRateOverride !== null
        ...
        : 0.20;
      const commissionInCents = calculateCommissionCents(valueInCents, commissionRate);
    ...
    const stats = {
      ...
      pendingReferrals,
      approvedReferrals,
      totalRevenue: totalRevenue._sum?.amountCents || 0, // Confirmed revenue from approved conversions
