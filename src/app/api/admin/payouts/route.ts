...
async function verifyAdmin(request: NextRequest) {
  try {
    ...
    if (!user || (user.role as string !== 'ADMIN') || user.status !== 'ACTIVE') {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedPayouts = payouts.map((payout: any) => ({
      ...
      affiliateName: payout.affiliate.user?.name ?? null,
      affiliateEmail: payout.affiliate.user?.email ?? null,
      payoutDetails: payout.affiliate.payoutDetails,
