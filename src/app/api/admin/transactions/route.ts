...
export async function POST(request: NextRequest) {
  try {
    ...
    let commissionRate = normalizeCommissionRate(affiliate.commissionRateOverride);

    if (affiliate.commissionRateOverride === null && affiliate.partnerGroupId) {
      const partnerGroup = await prisma.partnerGroup.findUnique({
        where: { id: affiliate.partnerGroupId }
      });
      if (partnerGroup) {
        commissionRate = normalizeCommissionRate(partnerGroup.commissionRate);
      }
    }
