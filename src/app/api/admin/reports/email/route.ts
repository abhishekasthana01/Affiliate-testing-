...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    <body>
      ...
      <div class="footer">
        <p>This report was sent from Beam Affiliate by ${user.name} (${user.email})</p>
        <p>© ${new Date().getFullYear()} Beam Affiliate. All rights reserved.</p>
      </div>
    </body>
    </html>
    `;
    ...
    const fromEmail = process.env.SMTP_FROM || process.env.RESEND_FROM_EMAIL || 'Beam Affiliate <noreply@beamaffiliate.com>';
    const results = await Promise.allSettled(
      recipients.map((email: string) =>
        mailer.send({
          from: fromEmail,
          to: email.trim(),
          subject: `[Beam Affiliate] ${reportData.type || 'Report'} — ${reportDate}`,
          html,
          attachments: [
            {
              filename: `${reportType}_report_${reportDate.replace(/\s+/g, '_')}.csv`,
              content: csvContent,
            },
          ],
        })
