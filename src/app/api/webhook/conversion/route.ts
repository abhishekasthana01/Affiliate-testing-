...
export async function POST(request: NextRequest) {
  try {
    ...
    const signature = request.headers.get('x-webhook-signature') || request.headers.get('x-beam-signature');
    ...
    const rawCommissionValue = applicableRule?.value || 15;
    const commissionRate = applicableRule?.type === 'PERCENTAGE'
      ? normalizeCommissionRate(rawCommissionValue)
      : rawCommissionValue;
    let commissionAmount = 0;

    if (applicableRule?.type === 'PERCENTAGE' && amount_cents) {
      commissionAmount = calculateCommissionCents(amount_cents, commissionRate);
    } else if (applicableRule?.type === 'FIXED') {
      commissionAmount = Math.floor(rawCommissionValue);
    }
