...
class AuthService {
  ...
  async register(data: RegisterData): Promise<{ success: boolean; message: string; user?: User; affiliate?: any }> {
    try {
      ...
      // Set to ACTIVE by default so they can log in immediately and appear in Admin Partners
      ...
      const user = await prisma.user.create({
        data: {
          ...
          beamNumber: data.beamNumber, // Add beamNumber to user record if it exists
        }
      });
      ...
      if (userRoleLower === 'affiliate') {
        ...
        affiliate = await prisma.affiliate.create({
          data: {
            ...
            resellerId,
            referralCode,
            payoutDetails: data.beamNumber ? { beamNumber: data.beamNumber } : {},
            balanceCents: 0
          }
        });
        ...
        try {
          ...
          if (adminIds.length > 0) {
            await notificationService.notifyAffiliateRegistered(adminIds, {
              name: user.name,
              email: user.email
            });
          }
