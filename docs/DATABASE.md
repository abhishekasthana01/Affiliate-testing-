...
Complete reference for Beam's PostgreSQL database schema using Prisma ORM.

## Overview

Beam uses PostgreSQL with Prisma ORM for type-safe database access. The schema is designed for a multi-tenant affiliate marketing platform.
...
| `productName` | String | Product/service name | "Product Name" | "Beam" |
| `programName` | String | Program display name | "Affiliate Program" | "Beam Partners" |
| `websiteUrl` | String | Main website URL | "https://example.com" | "https://beam.com" |
...
async function main() {
  ...
  await prisma.programSettings.create({
    data: {
      productName: 'Beam',
      programName: 'Beam Partners',
      websiteUrl: 'https://beam.com',
      currency: 'USD',
...
**Database Documentation by Beam Team**
